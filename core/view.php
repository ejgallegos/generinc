...
abstract class View {
    ...
    function render_template_intranet($contenido) {
        $dict = array("{app_nombre}"=>APP_TITTLE,
                      "{app_version}"=>APP_VERSION,
                      "{app_footer}"=>APP_TITTLE . " " . date("Y"),
                      "{contenido}"=>$contenido);
        $post_dict = array("{url_app}"=>URL_APP, "{url_static}"=>URL_STATIC_INTRANET, "{url_static_backend}"=>URL_STATIC);
        $plantilla = file_get_contents(TEMPLATE_INTRANET);
